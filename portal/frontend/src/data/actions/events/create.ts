...
import { z } from 'zod'
import { postEventService } from '@/data/services/events'
...
export async function createEventAction(prevState: any, formData: FormData) {
	...
	eventData.append('is_attestation', formData.get('is_attestation') ? 'true' : 'false')
	eventData.append('is_seminar', formData.get('is_seminar') ? 'true' : 'false')
	...
	const responseData = await postEventService(eventData)
	...
	redirect(`/events/${responseData.slug}/`)
}
